...
CREATE TABLE IF NOT EXISTS public.raw_products (
    ...
    product_sku VARCHAR(50) UNIQUE NOT NULL CHECK (LENGTH(product_sku) >= 3),
    product_name VARCHAR(255) NOT NULL CHECK (LENGTH(product_name) >= 1),
    category_id INTEGER CHECK (category_id > 0),
    unit_price NUMERIC(10, 2) NOT NULL CHECK (unit_price >= 0),
    current_stock INTEGER NOT NULL DEFAULT 0 CHECK (current_stock >= 0),
    is_active BOOLEAN NOT NULL DEFAULT TRUE,
    created_timestamp TIMESTAMPTZ NOT NULL DEFAULT NOW() CHECK (created_timestamp <= NOW())
);
...
CREATE TABLE IF NOT EXISTS public.raw_orders (
    ...
    order_timestamp TIMESTAMPTZ NOT NULL DEFAULT NOW() CHECK (order_timestamp <= NOW()),
...
CREATE INDEX idx_raw_orders_status ON public.raw_orders (order_status);
